...
function Panel({ items: itemsProp }: { items?: NotifItem[] }) {
  const [items, setItems] = useState<NotifItem[]>(itemsProp ?? ITEMS);

  useEffect(() => {
    if (itemsProp !== undefined) setItems(itemsProp);
  }, [itemsProp]);

  const dismiss = (id: string) => setItems((cur) => cur.filter((it) => it.id !== id));
...
type NotificationsProps = { bare?: boolean; items?: NotifItem[]; floating?: boolean };

export default function Notifications({ bare = false, items, floating = false }: NotificationsProps) {
