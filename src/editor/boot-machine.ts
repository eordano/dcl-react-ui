...
export function bootReducer(state: BootState, event: BootEvent): BootState {
  switch (event.type) {
    ...
    case "progress": {
      ...
      if (state.phase === "idle") return { ...state, progress: pct };
      if (pct >= 100) {
        if (state.phase === "booting")
          return { ...state, progress: pct, phase: "handshaking" };
...
export function bootOverlay(state: BootState): BootOverlay {
  switch (state.phase) {
    ...
    case "handshaking":
      return { show: true, kind: "loading", text: "Starting scene…" };
    case "booting":
    default:
      return {
        show: true,
        ...
        text:
          state.progress == null
            ...
            : state.progress >= 100
              ? "Starting scene…"
              : `Loading scene editor… ${state.progress}%`,
      };
  }
}
