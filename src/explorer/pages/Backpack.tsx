...
type LoadoutEntry = { slot: number; urn: string; name?: string };

type ColorCat = "skin" | "hair" | "eyes";
...
const iconFor = (id: string): string => ICON_ID[id] ?? id;
...
function creatorInfo(w: Wearable): { label: string; url: string } | null {
...
const SORT_MENU_STYLE: CSSProperties = {
...
function sortItemStyle(active: boolean): CSSProperties {
  return {
            ...
            <div className="bp__browse">
              ...
              {kind === "wearables" ? (
                <nav className="bp__rail" aria-label="Categories">
                  {SLOTS.map((s) => {
                    const eqThumb = equippedByCat[s.id]?.thumbnail;
              ...
              <div className="bp__gridcol">
                ...
                {kind === "wearables" && COLOR_CATS.has(cat) ? (
                  ...
                  >
                    {(PALETTE[cat] ?? []).map((hex) => {
                      const isSel =
                        (curColors[cat as ColorCat] || "").toLowerCase() ===
                        hex.toLowerCase();
                      return (
                        <button
                          key={hex}
