...
const SKIN_COLORS: [string, string, string, string, string, string] = ["#f5d6c0", "#e8b48c", "#c98c63", "#8d5a3c", "#5c3824", "#3b2415"];
const EYE_COLORS: [string, string, string, string, string, string] = ["#4a3829", "#6b4f2a", "#37516b", "#3a6e4a", "#5a5a5a", "#7d3a3a"];
const HAIR_COLORS: [string, string, string, string, string, string] = ["#1a1a1a", "#3a2418", "#5c3824", "#b06a2c", "#d9a441", "#9b2d2d"];
...
function SidebarItem({
  ...
  const imgStyle: CSSProperties & { "--rb": string } = {
...
function AttrSelect({ label, value }: { label: string; value: string }) {
  return (
    <div className="bdie__attr">
...
export default function ChItemEditor({
  ...
  const wired = collection !== undefined;
  const noCollection = wired && !collection.id;
  const itemsUnknown = wired && !noCollection && realItems === undefined;
  const [tab, setTab] = useState("items");
  const [selectedItemId, setSelectedItemId] = useState(wired ? "" : "i1");
  const [fixtureItems, setFixtureItems] = useState<Item[]>(ITEMS);
  ...
  const isReviewing = !wired && mode === "reviewing";

  const shownItems: Item[] = wired
    ? (realItems ?? []).map((i) => ({
        id: i.id,
        name: i.name,
        type: i.type,
  ...
  const selectedCollection: Collection = wired
    ? {
        id: collection.id ?? "",
        name: collection.name,
        itemCount: collection.itemCount ?? (realItems ? realItems.length : undefined),
        status: collection.status,
      }
    : COLLECTIONS[0];

  const shownCollections: Collection[] = wired
    ? (collections ?? []).map((c) => ({
        id: c.id ?? c.name,
        name: c.name,
  ...
  const detailsImgStyle: CSSProperties & { "--rb": string } = {
    "--rb":
      "var(" + (RARITY_VAR[selectedItem?.rarity ?? "common"] || "--rar-common") + ")",
  };

  function toggleVisible(id: string) {
    setFixtureItems((arr) =>
      arr.map((i) => (i.id === id ? { ...i, visible: !i.visible } : i)),
    );
  }
  ...
  return (
    <div className={"bdie" + (isReviewing ? " is-reviewing" : "")}>
      {!wired && (
        <div className="bdie__demo-note" role="note">
          Demo data — sample collection for layout preview, not your wearables
        </div>
      )}
      {isReviewing && (
                ...
                <div className="bdie__collections">
                  {shownCollections.map((c) => (
                    <SidebarCollection
                      ...
                      onClick={wired ? undefined : () => setTab("items")}
                    />
                  ))}
                </div>
              ) : (
