...
type PillId = "graphics" | "sounds" | "controls" | "chat";

type SettingModuleBase = { title: string; setting?: string; fullscreen?: boolean };
type SettingModule = SettingModuleBase &
  (
    | { kind: "toggle"; on?: number; off?: number; defaultChecked?: boolean }
    | {
        kind: "slider";
        defaultValue?: number;
        min?: number;
    ...
    | { kind: "dropdown"; options: string[]; values?: number[]; defaultValue?: string }
  );
type SettingGroup = { title: string; modules: SettingModule[] };

const pct = (v: number): string => Math.round(v) + "%";

function emitSetting(m: SettingModule, raw: boolean | number | string) {
  if (m.fullscreen) {
    try {
...
const ALL_PILLS: { id: PillId; label: string }[] = [
...
const CONTENT: Partial<Record<PillId, SettingGroup[]>> = {
  graphics: [
...
const PILLS = ALL_PILLS.filter((p) => (CONTENT[p.id] ?? []).length > 0);

function Module({ m }: { m: SettingModule }) {
  return (
