...
const SCAFFOLD_ERROR_HINTS: { code: RegExp; message: string }[] = [
  { code: /ENOENT/, message: "Folder not found — it may have been moved or deleted." },
  { code: /EACCES|NotAllowedError|SecurityError/, message: "Permission denied — grant write access to the folder and try again." },
  ...
  { code: /AbortError|canceled|cancelled/, message: "Scene creation was cancelled — no folder was chosen." },
];
    ...
    <div className="create-project-wizard" data-step={step}>
      ...
      {view === "scaffolding" && (
        <div className="create-project-wizard__scaffold">
          ...
          {folderCancelled && (
            <p className="create-project-wizard__note" role="note">
              Folder selection cancelled — files will download instead.
            </p>
          )}
        </div>
      )}
        ...
        <div className="create-project-wizard__created-banner" role="status">
          ...
          <p className="create-project-wizard__created-sub">
            {!result ? (
              ...
              <>
                <strong>{result.files.length} files</strong>{" "}
                (<span title={result.folder}>{result.folder}/</span>) were
                downloaded to your computer. Open the scene in the web editor to
                start building.
              </>
            ) : (
              <>
          ...
          {folderCancelled && (
            <p className="create-project-wizard__note" role="note">
              Folder selection was cancelled, so the files downloaded instead of
              writing to a folder in place. Unzip them wherever you like, then open
              the scene in the web editor.
            </p>
          )}
          {!signedIn && (
