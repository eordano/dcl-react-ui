  ...
  if (node.kind === "dir") {
    return (
      <>
        ...
        >
          <span className="decode-caret">{collapsed ? "▸" : "▾"}</span> {node.name}
        </button>
        {!collapsed &&
          node.children?.map((c) => (
...
export default function DeCodeWorkspace({ code = {}, onClose }: DeCodeWorkspaceProps) {
  const { typesUrl = null, virtualFiles = [], getDir = null } = code;
  const hostRef = useRef<HTMLDivElement | null>(null);
  const editorRef = useRef<IStandaloneCodeEditor | null>(null);
  const monacoRef = useRef<Monaco | null>(null);
  const dirRef = useRef<FileSystemDirectoryHandle | null>(null);
  const virtualsRef = useRef<Map<string, string>>(new Map());
  const [status, setStatus] = useState<string | null>("Loading editor…");
    ...
    (async () => {
      try {
        ...
        if (dir) {
          ...
          for (const f of virtualFiles) virtualsRef.current.set(f.path, f.text);
          setSource("virtual");
          setTree(virtualTree(virtualFiles));
        }
        setStatus(null);
        const first = virtualFiles.find((f) => f.path === "src/index.ts") ? "src/index.ts" : null;
        if (dir) {
  ...
  async function createFile(rawPath: string): Promise<void> {
    ...
    if (dirRef.current) {
      ...
      if (!virtualsRef.current.has(path)) virtualsRef.current.set(path, starterFor(path));
      ...
      <div className="decode-side">
        <div className="decode-side-head">
          ...
          <span className={`decode-src decode-src-${source}`}>
            {source === "disk" ? "project folder" : "unsaved (in-memory)"}
          </span>
          <button
            type="button"
        ...
        <div className="decode-side-foot">
          {typesLoaded > 0 ? `TS ready · ${typesLoaded} SDK libs` : "TS ready"}
        </div>
      </div>
      <div className="decode-main">
        <div className="decode-bar">
          ...
          <button type="button" className="decode-btn" onClick={saveCurrent} disabled={!openPath}>
            Save{source === "virtual" ? " (memory)" : ""}
          </button>
          <button type="button" className="decode-btn" onClick={onClose}>
            Close
          </button>
        </div>
