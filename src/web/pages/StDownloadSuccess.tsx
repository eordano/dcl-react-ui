...
const Subtitle = ({ action }: { action: ReactNode }) => (
  <>
...
const StepMedia = ({ img }: { img: string }) => (
  <img className="stdownloadsuccess__media" src={asset(img)} alt="" aria-hidden="true" loading="lazy" />
);

type StDownloadSuccessProps = {
  os?: OSKey;
  loading?: boolean;
  progress?: number | null;
};

export default function StDownloadSuccess({ os = "macos", loading = false, progress = null }: StDownloadSuccessProps) {
      ...
      <main className="stdownloadsuccess">
        ...
        <div className="stdownloadsuccess__footer">
          <p className="stdownloadsuccess__footertext">
            {L.footerPrefix}{" "}
            <a className="stdownloadsuccess__retry" href={siteUrl("/download")}>
              {L.footerLinkLabel}
            </a>
          </p>
        </div>
