...
function Btn({ icon, label, active, tile, badge, badgeKind, dot, notifDot, to, onClick }: BtnProps) {
  return (
    <button
      className={
        "sb__btn" +
...
const LOWER: NavItem[] = [
  ...
  { icon: "emote", label: "Emotes" },
  { icon: "friends", label: "Friends", div: true, to: "Explorer/Pages/Friends" },
  { icon: "chat", label: "Chat", to: "Explorer/Frames/Chat" },
];
  ...
  return (
    <div className="sb__stage">
      <nav className="sb" aria-label="Main menu">
        ...
        >
          {ICONS.bell}
          {unread > 0 ? <span className="sb__notif" /> : null}
        </button>
          ...
          {LOWER.map((b) => (
            <span key={b.icon} className="sb__item">
              ...
              {b.icon === "chat" ? (
                ...
                <Btn icon="emote" label="Emotes" active={emoteOpen} onClick={onEmoteToggle} />
              ) : b.icon === "friends" ? (
