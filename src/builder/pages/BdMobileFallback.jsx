...
function ListIcon({ id }) {
  const common = {
    ...
    fill: "none",
    "aria-hidden": true,
    className: "bdmobilefallback__icon",
  };
  switch (id) {
    case "dress":
      return (
...
function ToolsCircle() {
  return (
    <svg
      className="bdmobilefallback__slot-image"
      width="96"
      height="96"
      viewBox="0 0 96 96"
      fill="none"
...
export default function BdMobileFallback() {
  return (
    <BuilderChrome active="" account="">
      <div className="bdmobilefallback">
        <div className="bdmobilefallback__container">
          <ToolsCircle />

          <h2 className="bdmobilefallback__title">
            Builder Tools are only available on desktop
          </h2>

          <span className="bdmobilefallback__list-title">
            From your computer, you can:
          </span>

          <ul className="bdmobilefallback__list">
            {LIST_ITEMS.map((item) => (
              <li key={item.id} className="bdmobilefallback__item">
                <ListIcon id={item.id} />
                <div className="bdmobilefallback__content">
                  {item.pre}
                  <b>{item.bold}</b>
                  {item.post}
                </div>
          ...
          <a
            className="bdmobilefallback__button"
