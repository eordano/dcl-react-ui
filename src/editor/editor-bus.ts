...
export interface PlayStateAnnouncement {
  type: "play-state";
  playing: boolean;
  paused: boolean;
}
...
type Ui3ExtraPageToScene =
  | { type: "set-camera"; mode: EditorCamMode; axis?: string }
  | { type: "undo" }
  | { type: "redo" }
  | {
      type: "add-entity";
...
type SendableMessage = PageToSceneMessage | Ui3ExtraPageToScene;

interface IncomingEnvelope {
  to?: unknown;
  msg?: SceneToPageMessage;
}
...
export function createEditorBus(): EditorBus {
  ...
  channel.onmessage = (ev: MessageEvent) => {
    const env = (ev?.data ?? null) as IncomingEnvelope | null;
  ...
  const post = (msg: SendableMessage) => {
    try {
  ...
  const bus: EditorBus = {
    ...
    setCamMode(mode) {
      if (EDITOR_CAM_MODES.has(mode)) post({ type: "set-camera", mode });
    },
    setCameraInput(d) {
    ...
    orientAxis(mode, axis) {
      if (EDITOR_CAM_MODES.has(mode) && typeof axis === "string") {
        post({ type: "set-camera", mode, axis });
      }
    },
    toggleOrtho() {
    ...
    addEntity(name, parent = 0, components = null, composite = null) {
      post({
        type: "add-entity",
        name: String(name || "Entity"),
        parent: Number(parent) || 0,
        components: components && typeof components === "object" ? components : null,
        composite: composite && typeof composite === "object" ? composite : null,
      });
    },
    ...
    announcePlayState(playing, paused) {
      const msg: PlayStateAnnouncement = { type: "play-state", playing: !!playing, paused: !!paused };
      try {
        channel.postMessage({ to: "page", msg });
      } catch {
      }
    },
