  ...
  return (
    <div className="mpw" data-step={step} data-state={value}>
      ...
      <div className="mpw-layer" role="group" aria-label="Moderation wizard">
        {value === "authGate" && (
          <div className="mpw-panel mpw-panel--gate" role="region" aria-label="Admin sign in">
            <h2 className="mpw-panel__title">Admin sign-in required</h2>
            <p className="mpw-panel__text">
              The Places report queue (<code>GET /api/reports</code>) is admin-bearer
              gated (<code>require_admin_bearer</code>, fail-closed 403).
              <br />
              <em>
                (Sign-in here only reveals the console; the queue read and the
                moderation writes are real admin-gated calls that fail closed
                without a bearer.)
              </em>
            </p>
            <button
