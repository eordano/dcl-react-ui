...
describe("tool letters (Q/W/E/R)", () => {
  it.each([
    ...
    ["e", "rotate"],
    ["r", "scale"],
  ])("%s → %s in the static edit camera", (k, tool) => {
  ...
  it("ignore modifier combos (⌘W must stay close-tab, Alt+E is not a tool)", () => {
...
describe("edit keys", () => {
  it("⌘Z / Ctrl+Z → undo, with ⇧ → redo — in every camera state, even playing", () => {
  ...
  it("Delete / Backspace → delete selection; modified variants pass through", () => {
