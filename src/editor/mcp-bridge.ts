...
const BUS_CHANNEL = "dcl-editor-bus";
...
interface BusEnvelope {
  to?: unknown;
  msg?: { type?: unknown; [k: string]: unknown };
}
...
export function connect(opts: ConnectOptions): () => void {
  ...
  channel.onmessage = (ev: MessageEvent) => {
    ...
    if (msg.type === "scene-ready") {
      sceneReady = true;
      bridgeVersion = typeof msg["bridge"] === "number" ? (msg["bridge"] as number) : null;
      const scene = msg["scene"] as { hash?: unknown } | null | undefined;
      sceneHash = scene && typeof scene.hash === "string" ? scene.hash : null;
      frozen = typeof msg["frozen"] === "boolean" ? (msg["frozen"] as boolean) : null;
    }
    if (msg.type === "rpc-reply" && typeof msg["id"] === "string" && rpcPending.has(msg["id"] as string)) {
      const entry = rpcPending.get(msg["id"] as string);
      rpcPending.delete(msg["id"] as string);
      if (entry) {
  ...
  const placeAssetPrep = async (
    ...
    const cache = contentBase && typeof caches !== "undefined" ? await caches.open("ch-project-v1").catch(() => null) : null;
