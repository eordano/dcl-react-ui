...
import { sendBridge, useBridgeState, FALLBACK_STATE } from "../../overlay/bridge";
import "./chat.css";
...
export default function Chat({ bare = false }: { bare?: boolean }) {
  ...
  const lines: ChatLine[] = chat.filter((l) => !FALLBACK_STATE.chat.includes(l));

  const arrivalRef = useRef<Map<string, number>>(new Map());
  function arrivalTime(line: ChatLine, i: number) {
    const key =
      line.timestamp != null
        ? `t${line.timestamp}`
