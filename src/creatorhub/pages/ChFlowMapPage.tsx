...
function edgeAria(e: FlowEdge): string {
  switch (e.kind) {
    ...
    case "load":
      return e.label
        ? `Click: ${e.label} — then a load over 100 milliseconds: ${e.work ?? "loading"}`
...
function SectionView({ section }: { section: FlowSection }) {
  return (
        ...
        {section.machines && (
          <span className="fm-machines">
            {section.machines.map((m) => (
              <code key={m} title={`sites/app/stories/creator-hub/${m}/machine.ts`}>
                {m}
              </code>
            ))}
          </span>
...
function Legend() {
  return (
    <div className="fm-legend" aria-label="Legend">
      ...
      <div className="fm-legend-item">
        <EdgeView
          edge={{ t: "edge", kind: "load", label: "click", work: "“Loading…”" }}
          chains={[]}
          demo
        />
        <span>
...
function AsciiDetails() {
  const [copied, setCopied] = useState(false);
  const copy = () => {
    if (typeof navigator !== "undefined" && navigator.clipboard) {
      navigator.clipboard.writeText(ASCII_SOURCE).then(
        () => {
          setCopied(true);
  ...
  return (
    <details className="fm-ascii">
      ...
      <div className="fm-ascii-body">
        ...
        <pre tabIndex={0} aria-label="Creator Hub sitemap, ascii original">
          {ASCII_SOURCE}
        </pre>
      </div>
    </details>
      ...
      <div className={`fmp${active ? " fmp--dim" : ""}`}>
        ...
        <header className="fm-hero">
          <div className="fm-hero-top">
            {LinkComponent ? (
              <LinkComponent to="/create" className="fm-back">
                ← Creator Hub
              </LinkComponent>
            ) : (
              <a href="/create" className="fm-back">
                ← Creator Hub
              </a>
            )}
            <span className="fm-crumb">sites / creator-hub / map</span>
          </div>
          ...
          <p className="fm-lede">
            ...
            anything to light its full path. Verified against the 2026-07-04 story audit.
          </p>
        ...
        <footer className="fm-foot">
          <AsciiDetails />
          <p className="fm-honesty">
            Source of truth:{" "}
            <code>sites/app/stories/CREATOR-HUB-SITEMAP.md</code> · machine states from{" "}
            <code>sites/app/stories/creator-hub/*/machine.ts</code>. Intentionally absent
            — not implemented: <code>metrics-funnel</code>,{" "}
            <code>integration-create-entry</code>, single items. Publish&nbsp;Pay is a
            disclosed stub; world-permissions commit is a disclosed simulation;
            Unpublish is real.
          </p>
        </footer>
      </div>
