import "./streelsuserlist.css";

const NoCameraGlyph = () => (
  <svg className="streelsuserlist__nocamera" viewBox="0 0 120 120" fill="none" aria-hidden="true">
    <g opacity="0.8">
      <path
...
function NotPhoto() {
  return (
    <div className="streelsuserlist streelsuserlist--empty">
      <NoCameraGlyph />
      <h1 className="streelsuserlist__emptytitle">Photo not found</h1>
      <h3 className="streelsuserlist__emptysub">
...
export default function StReelsUserList({
  address = "0x9a8e7d6c5b4a3f2e1d0c9b8a7f6e5d4c3b2a1f0e",
  ...
  if (isLoading) {
    return (
      <div className="streelsuserlist streelsuserlist--loading" aria-busy="true">
        <span className="streelsuserlist__titleskel" aria-hidden="true" />
        <div className="streelsuserlist__grid">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="streelsuserlist__item streelsuserlist__item--skel" aria-hidden="true" />
          ))}
        </div>
        <span className="streelsuserlist__sr">Loading photos…</span>
      </div>
    );
  }
  ...
  return (
    <div className="streelsuserlist">
      <h2 className="streelsuserlist__title">User: {userName}</h2>
      <div className="streelsuserlist__grid">
        {images.map((image) => (
          <div
            key={image.id}
            className="streelsuserlist__item"
            role="button"
            tabIndex={0}
...
const THUMB = "https://decentraland.org/images/decentraland-social-share.png";
const SAMPLE_USER = {
...
const SAMPLE_IMAGES = Array.from({ length: 24 }, (_, i) => ({
